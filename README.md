...
A complete, runnable Python application that acts as an intelligent router for user intents. It leverages Google's Gemini LLM to classify user requests into predefined intents and subsequently queries a specialized expert prompt to generate a highly aligned, high-quality response.
...
## Important Note
This project uses the **Google Gemini API** (`gemini-2.0-flash`). You must provide a valid `GEMINI_API_KEY` to run the application.
...
2. **Router & Generator `router.py`**: Uses the extracted intent to select an `EXPERT_PROMPT` (from `prompts.py`). It passes the user message to the Gemini API with the expert system instruction, yielding high-quality, persona-specific completions.
